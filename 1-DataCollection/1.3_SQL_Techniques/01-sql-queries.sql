...
select  sum(name) as total_words from books group by name having total_words > 1000000;

/* author whose average book is over 150,000 words */
select  avg(name) as avg_words from books group by name having avg_words > 150000;
...
INSERT INTO student_grades1 (name, number_grade, fraction_completed)
...
select name, number_grade, round(100.00 * fraction_completed) as percent_completed from student_grades1;
